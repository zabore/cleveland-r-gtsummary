...
cranlogs::cran_downloads(packages = "gtsummary", from = "2019-08-01") %>% 
  mutate(
    newdate = as.Date(lubridate::parse_date_time(format(date, "%m/%Y"), "my"))
  ) %>% 
  group_by(newdate) %>% 
  summarize(avg_dls = mean(count)) %>% 
  ggplot(aes(x = newdate, y = avg_dls)) +
  geom_line() +
...
.medium[
`r icon::fa("twitter")`: @zabormetrics,    `r icon::fa("github")`: @zabore
]
